 ...
 * Å° The new owner of this mutex should make the state protected by the mutex consistent.
 * This state might have been left inconsistent when the previous owner terminated.
 * Å° If the new owner is able to make the state consistent, call
 ...
 * Å° If the new owner is not able to make the state consistent, do not call
 ...
 * Å° If the thread that acquires the lock with EOWNERDEAD terminates without unlocking the
